...
class MinIO(object):
    ...
    def __init__(self, config, pathTmp=None, verbose=True):
        """ Initialising object
        Parameters
        ----------
        config : dict
    ...
    def uploadFolder(self, bucketName, folder_tmp, folderPath):
        try:
            for diretorio, subpastas, arquivos in os.walk(join(folder_tmp, folderPath)):
                for arquivo in arquivos:
                    fullPath = join(realpath(diretorio), arquivo)
                    self.uploadFile(bucketName, join(folderPath, arquivo), fullPath)

        except Exception as e:
            print("FATAL: uploadFolder to Minio(%s): %s" % (self.url, str(e)))
            return False
        return True
    ...
    def uploadFile(self, bucketName, filename, fullPath):
        try:
            ...
            self.handle.fput_object(bucketName, filename, fullPath, metadata={'key0': 'value0', 'key1': 'value1'})
        except Exception as e:
    ...
    def getObj(self, bucketName, source):
        try:
            response = self.handle.get_object(bucketName, source)
